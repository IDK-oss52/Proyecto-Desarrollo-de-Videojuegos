...
private:
	...
	void Draw(float dt);
	void UpdateCamera();
	void DrawDeathSequence(float dt); 
...
private:
	...
	SDL_Texture* deathFlashTex = nullptr;     
	SDL_Texture* deathAbsorbTex = nullptr;     // 384x32 (11 frames de ~35x32)
	SDL_Texture* youDiedTex = nullptr;         

	 
	Animation animDeathFlash;                   
	Animation animDeathAbsorb;                  

	float deathTimer = 0.0f;                 
	int deathStage = 0;                       
	bool deathSequenceStarted = false;

	const float kFlashDuration = 400.0f;       
	const float kAbsorbDuration = 1000.0f;   
	const float kFallDuration = 800.0f;        
	const float kYouDiedDuration = 1500.0f;    

	float fallingSpeed = 200.0f;               
	float youDiedAlpha = 0.0f;                 
};
