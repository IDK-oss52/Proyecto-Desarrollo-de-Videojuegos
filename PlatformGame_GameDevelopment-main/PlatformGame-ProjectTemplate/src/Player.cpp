...
bool Player::Start() {
	...
	LOG("Loading death animation assets...");

	deathFlashTex = Engine::GetInstance().textures->Load("Assets/Textures/playerDie1_spritesheet.png");
	deathAbsorbTex = Engine::GetInstance().textures->Load("Assets/Textures/playerDie2_spritesheet.png");
	youDiedTex = Engine::GetInstance().textures->Load("Assets/Textures/you_died.png");

	
	if (deathFlashTex) {
		int texW = 0, texH = 0;
		Engine::GetInstance().textures->GetSize(deathFlashTex, texW, texH);
		LOG("Flash texture size: %dx%d", texW, texH); 
		for (int i = 0; i < 4; i++) {
			animDeathFlash.PushBack({ i * 32, 0, 32, 32 });
		}
		animDeathFlash.speed = 0.8f;
		animDeathFlash.loop = false;
	}

	// Absorb (11x32)
	if (deathAbsorbTex) {
		int texW = 0, texH = 0;
		Engine::GetInstance().textures->GetSize(deathAbsorbTex, texW, texH);
		LOG("Absorb texture size: %dx%d", texW, texH); 
		for (int i = 0; i < 11; i++) {
			animDeathAbsorb.PushBack({ i * 32, 0, 32, 32 });
		}
		animDeathAbsorb.speed = 0.05f;
		animDeathAbsorb.loop = false;
	}

	LOG("Death animation assets loaded");
...
bool Player::Update(float dt)
{
	if (isDead) {
		...
		if (deathStage == 1) {
			animDeathFlash.Update(dt);

			if (deathTimer >= kFlashDuration) {
				deathStage = 2;
				deathTimer = 0.0f;
			}
		}
		else if (deathStage == 2) {
			animDeathAbsorb.Update(dt);

			if (deathTimer >= kAbsorbDuration) {
				deathStage = 3;
				deathTimer = 0.0f;
			}
		}
		else if (deathStage == 3) {
			float deltaY = fallingSpeed * (dt / 1000.0f);
			position.setY(position.getY() + deltaY);

			if (pbody) {
				pbody->SetPosition((int)position.getX(), (int)position.getY());
			}

			if (deathTimer >= kFallDuration) {
				deathStage = 4;
				deathTimer = 0.0f;
				youDiedAlpha = 0.0f;
			}
			...
			youDiedAlpha += (dt / kYouDiedDuration);
			if (youDiedAlpha > 1.0f) youDiedAlpha = 1.0f;

			if (deathTimer >= kYouDiedDuration) {
				Respawn();
...
void Player::Die() {
	...
	LOG("=== PLAYER DIED ===");
	LOG("Starting death animation sequence...");
}
...
void Player::DrawDeathSequence(float dt) {
	int x, y;
	if (pbody) {
		pbody->GetPosition(x, y);
	}
	...
	if (deathStage == 1) {
		if (deathFlashTex) {
			const SDL_Rect& frame = animDeathFlash.GetCurrentFrame();
		...
		if (deathAbsorbTex) {
			const SDL_Rect& frame = animDeathAbsorb.GetCurrentFrame();
			Engine::GetInstance().render->DrawTexture(
				deathAbsorbTex,
				x - frame.w / 2,
				y - frame.h / 2,
		...
		if (deathAbsorbTex) {
			const SDL_Rect& lastFrame = animDeathAbsorb.GetFrame(10);  
			Engine::GetInstance().render->DrawTexture(
				deathAbsorbTex,
				(int)position.getX() - lastFrame.w / 2,
				(int)position.getY() - lastFrame.h / 2,
				&lastFrame
			);
		}
	}
		...
		if (youDiedTex) {
			SDL_SetTextureAlphaMod(youDiedTex, (Uint8)(youDiedAlpha * 255.0f));

			int screenW = Engine::GetInstance().render->camera.w;
			int screenH = Engine::GetInstance().render->camera.h;

			int texW = 0, texH = 0;
			Engine::GetInstance().textures->GetSize(youDiedTex, texW, texH);

			int drawX = -Engine::GetInstance().render->camera.x + screenW / 2 - texW / 2;
			int drawY = -Engine::GetInstance().render->camera.y + screenH / 2 - texH / 2;

			Engine::GetInstance().render->DrawTexture(youDiedTex, drawX, drawY, nullptr);
		}
	}
}
...
bool Player::CleanUp()
{
	...
	if (deathFlashTex) Engine::GetInstance().textures->UnLoad(deathFlashTex);
	if (deathAbsorbTex) Engine::GetInstance().textures->UnLoad(deathAbsorbTex);
	if (youDiedTex) Engine::GetInstance().textures->UnLoad(youDiedTex);
