...
bool Animation::HasFinishedOnce() const {
    return finishedOnce_ && !loop_;
}

void Animation::Update(float dt) {
    if (frames_.empty()) return;

    timeInFrameMs_ += static_cast<int>(dt * speed);

    while (timeInFrameMs_ >= frames_[currentIndex_].durationMs) {
        timeInFrameMs_ -= frames_[currentIndex_].durationMs;

        bool isLastFrame = (currentIndex_ >= static_cast<int>(frames_.size()) - 1);
...
void Animation::PushBack(const SDL_Rect& rect) {
    frames_.push_back({ rect, 100 }); 
}

const SDL_Rect& Animation::GetFrame(int index) const {
    if (index >= 0 && index < static_cast<int>(frames_.size())) {
        return frames_[index].rect;
    }
    return kEmpty_;
}
