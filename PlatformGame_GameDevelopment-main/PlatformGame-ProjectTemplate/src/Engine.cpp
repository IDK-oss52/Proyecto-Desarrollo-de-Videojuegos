...
void Engine::DrawDebugHelp()
{
    ...
    int posX = 10 - render->camera.x;
    int posY = 10 - render->camera.y;

    if (helpMenuTexture) {
        render->DrawTexture(helpMenuTexture, posX, posY, nullptr, 1.0f, 0.0, INT_MAX, INT_MAX);
        return;
    }

    SDL_Rect bgRect = { posX, posY, 400, 200 };
    render->DrawRectangle(bgRect, 0, 0, 0, 200, true, true);
    render->DrawRectangle(bgRect, 255, 255, 0, 255, false, true);
}
