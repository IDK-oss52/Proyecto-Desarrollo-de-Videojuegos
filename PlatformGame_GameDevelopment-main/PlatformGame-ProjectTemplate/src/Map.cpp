...
bool Map::Load(std::string path, std::string fileName)
{
    ...
    if (result == NULL)
    {
        ...
        int halfW = mapData.tileWidth / 2;
        int halfH = mapData.tileHeight / 2;

        for (const auto& layer : mapData.layers) {
            // 1. Filtramos capas por nombre
            bool isCollisionLayer = (layer->name == "Collisions" || layer->name == "Map");
            bool isDamageLayer = (layer->name == "Damage");

            if (!isCollisionLayer && !isDamageLayer) continue;
            ...
            for (int y = 0; y < mapData.height; ++y) {
                for (int x = 0; x < mapData.width; ++x) {
                    ...
                    int tileId = layer->Get(y, x);

                    if (tileId == 0) continue;

                    Vector2D worldPos = MapToWorld(y, x);
                    int centerX = static_cast<int>(worldPos.getX()) + halfW;
                    int centerY = static_cast<int>(worldPos.getY()) + halfH;

                    if (isCollisionLayer) {
                        TileSet* tSet = GetTilesetFromTileId(tileId);

                        if (!tSet) continue;

                        if (tSet->name == "MapMetadata" || tSet->name == "Map") {
                            PhysBody* body = physics.CreateRectangle(centerX, centerY, mapData.tileWidth, mapData.tileHeight, STATIC);
                            body->ctype = ColliderType::PLATFORM;
                        }
                        else if (tSet->name == "MapData") {
                            PhysBody* body = physics.CreateRectangleSensor(centerX, centerY, mapData.tileWidth, mapData.tileHeight, STATIC);
                            body->ctype = ColliderType::PLATFORM_ONEWAY;
                        }
                    }
                    else if (isDamageLayer && tileId == 2) {
                        PhysBody* body = physics.CreateRectangle(centerX, centerY, mapData.tileWidth, mapData.tileHeight, STATIC);
